...
def plot_single_sample_initialization_comparison():
    ...
    if not np.array_equal(time_points_linear, time_points_no):
        print(
            "Warning: time_points arrays differ between loaded samples. Using the one from LINEAR_INITIALIZATION_TRAINED."
        )
    time_points = time_points_linear

    # --- Calculate Squared Errors ---
    squared_error_reflectance_linear = (
        predicted_reflectance_linear - true_reflectance
    ) ** 2
    squared_error_reflectance_neural = (
        predicted_reflectance_neural_operator - true_reflectance
    ) ** 2

    squared_error_thickness_linear = (predicted_thickness_linear - true_thickness) ** 2
    squared_error_thickness_neural = (
        predicted_thickness_neural_operator - true_thickness
    ) ** 2

    squared_error_growth_rate_linear = (
        predicted_growth_rate_linear - true_growth_rate
    ) ** 2
    squared_error_growth_rate_neural = (
        predicted_growth_rate_neural_operator - true_growth_rate
    ) ** 2
    ...
    gs_inner1 = gridspec.GridSpecFromSubplotSpec(
        2, 1, subplot_spec=gs_outer[0, 0], height_ratios=[2, 1], hspace=0.1
    )
    ax1_main = fig.add_subplot(gs_inner1[0])
    ax1_err = fig.add_subplot(gs_inner1[1], sharex=ax1_main)

    # --- Section 2: Thickness ---
    gs_inner2 = gridspec.GridSpecFromSubplotSpec(
        2, 1, subplot_spec=gs_outer[0, 1], height_ratios=[2, 1], hspace=0.1
    )
    ...
    gs_inner3 = gridspec.GridSpecFromSubplotSpec(
        2, 1, subplot_spec=gs_outer[0, 2], height_ratios=[2, 1], hspace=0.1
    )
    ...
    ax1_main.plot(
        time_points, true_reflectance, label="ground truth", color="black", linewidth=4
    )
    ax1_main.plot(
        initialized_time_points_linear,
        initialized_reflectance_linear,
        label="linear initialization (pre-trained)",
        color="blue",
        linestyle="--",
    )
    ax1_main.plot(
        initialized_time_points_neural_operator,
        initialized_reflectance_neural_operator,
        label="neural operator initialization",
        color="red",
        linestyle="--",
    )
    ax1_main.plot(
        time_points,
        predicted_reflectance_linear,
        label="optimization result after linear initialization (pre-trained)",
        color="blue",
    )
    ax1_main.plot(
        time_points,
        predicted_reflectance_neural_operator,
        label="optimization result after neural operator initialization",
        color="red",
    )
    ...
    # --- Plotting Thickness ---
    ax2_main.plot(
        time_points, true_thickness, label="ground truth", color="black", linewidth=4
    )
    ax2_main.plot(
        initialized_time_points_linear,
        initialized_thickness_linear,
    ...
    ax3_main.plot(
        time_points, true_growth_rate, label="ground truth", color="black", linewidth=4
    )
    ax3_main.plot(
        initialized_time_points_linear,
        initialized_growth_rate_linear,
    ...
    h_no_init = mlines.Line2D(
        [], [], color="red", linestyle="--", label="neural operator initialization"
    )
    h_opt_lin = mlines.Line2D(
        [],
        [],
        color="blue",
        label="optimization result after linear initialization (pre-trained)",
    )
    h_opt_no = mlines.Line2D(
        [],
        [],
        color="red",
        label="optimization result after neural operator initialization",
    ...
    legend_handles_ordered = [h_gt, h_dummy, h_lin_init, h_no_init, h_opt_lin, h_opt_no]
