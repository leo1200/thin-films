...
for initialization in [LINEAR_INITIALIZATION_TRAINED, NEURAL_OPERATOR_INITIALIZATION]:
    ...
    for sample_number in range(1, total_samples + 1):
        ...
        ax1.plot(epochs_ref_dec, ref_dec, color = initialization_colors[initialization], alpha = 0.1)
        ax2.plot(epochs_th_dec, th_dec, color = initialization_colors[initialization], alpha = 0.1)
        ax3.plot(epochs_gr_dec, gr_dec, color = initialization_colors[initialization], alpha = 0.1)
...
for initialization in [LINEAR_INITIALIZATION_TRAINED, NEURAL_OPERATOR_INITIALIZATION]:
    ...
    for loss_type, ax in zip(["reflectance", "thickness", "growth_rate"], [ax1, ax2, ax3]):
        ...
        ax.plot(epochs, medians, "--", label=f"median ({initialization_to_label_string(initialization)})", color=color, linewidth=1)
...
plt.savefig(f"figures/all_losses.svg")
