...
def plot_single_validation_result(initialization, sample_number):
    ...
    data = np.load(
        f"validation_results_data/{initialization_to_string(initialization)}/{initialization_to_string(initialization)}_{sample_number}.npz"
    )
    ...
    plt.savefig(
        f"validation_results_figures/{initialization_to_string(initialization)}/predictions_{initialization_to_string(initialization)}_{sample_number}.svg"
    )
    plt.close("all")
    ...
    plt.tight_layout()
    plt.savefig(
        f"validation_results_figures/{initialization_to_string(initialization)}/losses_{initialization_to_string(initialization)}_{sample_number}.svg"
    )
...
def plot_all_validation_results(total_samples=200):
    for initialization in initializations:
        for sample_number in range(1, total_samples + 1):
            ...
            print(
                f"produced plot for {initialization_to_string(initialization)}_{sample_number}"
            )
...
def plot_validation_loss_curves(total_samples=200, figpath="figures/loss_curves.svg"):
    ...
    for initialization in [
        ...
        for sample_number in range(1, total_samples + 1):
            ...
            ref_dec, epochs_ref_dec = find_strictly_smaller_than_previous_vectorized(
                reflectance_losses
            )
            th_dec, epochs_th_dec = find_strictly_smaller_than_previous_vectorized(
                thickness_losses
            )
            gr_dec, epochs_gr_dec = find_strictly_smaller_than_previous_vectorized(
                growth_rate_losses
            )
            ...
            if np.min(thickness_losses) < failure_thickness:
                for epoch, val in zip(epochs_ref_dec, ref_dec):
                    losses_by_init_and_epoch[initialization]["reflectance"][
                        epoch
                    ].append(val)
                for epoch, val in zip(epochs_th_dec, th_dec):
                    losses_by_init_and_epoch[initialization]["thickness"][epoch].append(
                        val
                    )
                for epoch, val in zip(epochs_gr_dec, gr_dec):
                    losses_by_init_and_epoch[initialization]["growth_rate"][
                        epoch
                    ].append(val)
    ...
    for initialization in [
        ...
        for loss_type_idx, (loss_type, ax) in enumerate(
            zip(["reflectance", "thickness", "growth_rate"], axes)
        ):
            ...
            smoothed_medians = np.convolve(
                medians, np.ones(smoothing_window) / smoothing_window, mode="valid"
            )
            adjusted_epochs = epochs[
                smoothing_window - 1 :
            ]  # This will also be empty if smoothed_medians is empty
    ...
    titles = [
        "Monotonized Reflectance Loss",
        "Monotonized Thickness Loss",
        "Monotonized Growth Rate Loss",
    ]
    ...
    plt.savefig(figpath, bbox_inches="tight", pad_inches=0.02)
...
def loss_analysis_plot(total_samples=200, figpath="figures/losses_kde.svg"):
    ...
    for initialization in initializations_list:  # Use the defined list
        for sample_number in range(1, total_samples + 1):
            ...
            if initialization == NEURAL_OPERATOR_INITIALIZATION:
                neural_operator_initial_losses_reflectance.append(
                    data["initial_reflectance_loss"]
                )
                neural_operator_initial_losses_thickness.append(
                    data["initial_thickness_loss"]
                )
                neural_operator_initial_losses_growth_rate.append(
                    data["initial_growth_rate_loss"]
                )

            # Logic for first_epoch_with_growth_rate_loss_below (kept as is)
            if "growth_rate_losses" in data:  # Check if key exists
                lower_than_barrier = (
                    data["growth_rate_losses"] < growth_rate_loss_barrier
                )
        ...
        reflectance_losses[initialization] = np.array(
            reflectance_losses[initialization]
        )
        thickness_losses[initialization] = np.array(thickness_losses[initialization])
        growth_rate_losses[initialization] = np.array(
            growth_rate_losses[initialization]
        )
        ...
        if total_samples > 0:  # Avoid division by zero if no samples loaded
            fraction_samples_above_100 = (
                num_samples_above_100 / len(thickness_losses[initialization])
                if len(thickness_losses[initialization]) > 0
                else 0
            )
        else:
            fraction_samples_above_100 = 0
        print(
            f"Initialization {initialization_to_string(initialization)}: {num_samples_above_100} samples above 100 thicknes MSE, fraction: {fraction_samples_above_100:.2f}"
        )
    ...
    x_labels_list = [
        "mean squared reflectance error (unitless)",
    ...
    h_no_guess = mlines.Line2D(
        [], [], color="black", label=legend_labels["NEURAL_OPERATOR_GUESS"]
    )
    h_opt_random = mlines.Line2D(
        [],
        [],
        color=initialization_colors[RANDOM_INITIALIZATION],
        label=legend_labels[RANDOM_INITIALIZATION],
    )
    h_opt_linear_set = mlines.Line2D(
        [],
        [],
        color=initialization_colors[LINEAR_INITIALIZATION_SET],
        label=legend_labels[LINEAR_INITIALIZATION_SET],
    )
    h_opt_linear_trained = mlines.Line2D(
        [],
        [],
        color=initialization_colors[LINEAR_INITIALIZATION_TRAINED],
        label=legend_labels[LINEAR_INITIALIZATION_TRAINED],
    )
    h_opt_no_init = mlines.Line2D(
        [],
        [],
        color=initialization_colors[NEURAL_OPERATOR_INITIALIZATION],
        label=legend_labels[NEURAL_OPERATOR_INITIALIZATION],
    )

    h_dummy = mlines.Line2D(
        [], [], color="none", marker="None", linestyle="None", label=" "
    )
